...
BOT_NICKNAME = "@Твой_Юзернейм_Бота" # Впиши сюда юзернейм своего бота
# -----------------
...
async def start_handler(message: types.Message):
    ...
    await message.answer(
        f"👋 Привет! Я AI Помощник.\n\n"
        f"📥 Пришли ссылку из TikTok/Reels — скачаю без знаков.\n"
...
async def process_download(callback: types.CallbackQuery):
    ...
    msg = await callback.message.edit_text(f"⏳ Качаю {choice}...")
    ...
    try:
        ...
        if choice == 'video':
            await callback.message.answer_video(types.FSInputFile(file_path), caption=caption_text)
        else:
            await callback.message.answer_audio(types.FSInputFile(file_path), caption=caption_text)
    except Exception:
        await callback.message.answer("❌ Ошибка загрузки. Ссылка может быть защищена.")
    finally:
        if os.path.exists(file_path): os.remove(file_path)
        await msg.delete()

async def main():
    await start_web_server() # Запуск "обманки" для порта
    scheduler = AsyncioScheduler()
    scheduler.add_job(auto_post_trend, "interval", hours=6) # Пост каждые 6 часов
    scheduler.start()
